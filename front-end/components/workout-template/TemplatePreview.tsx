...
export default function TemplatePreview({ template }: { template: Template }) {
  return (
    <aside className="rounded-lg border border-outline-variant bg-surface-container-low p-5">
      ...
      <h3 className="type-label-caps mb-3 mt-6 text-on-surface-variant">Sequência</h3>
      <div className="space-y-2">
        {template.exercises.map((exercise, index) => (
          ...
          >
            <span className="font-mono text-primary-fixed-dim">{String(index + 1).padStart(2, "0")}</span>
            <span className="text-on-surface-variant"><RiHeartPulseLine /></span>
