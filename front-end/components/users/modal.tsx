...
const createUserSchema = yup.object({
    ...
    passwordConfirmation: yup.string().required("Confirmação de senha é obrigatória").oneOf([yup.ref("password")], "As senhas não conferem."),
  ...
  const canSubmit = mode === "edit"
    ...
    async function handleSubmit(event: React.SubmitEvent<HTMLFormElement>) {
        ...
        if (mode === "edit" && props.user) {
            ...
            if (!result.success) {
                setErrors({ form: result.error || "Não foi possível atualizar o usuário." });
                return;
            }
            await props.onSaved();
            return;
    ...
    return (
        ...
        >
          ...
          {(mode !== "view") && <Button type="submit" disabled={!canSubmit}>
                        {mode === "edit" ? (isSubmitting ? "Salvando..." : "Salvar alterações") : (isSubmitting ? "Salvando..." : "Criar usuário")}
                    </Button>}
                </div>
            </form>
        </Modal>
