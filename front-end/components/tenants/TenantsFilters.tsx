...
export default function TenantsFilters(props: TenantsFiltersProps) {
	const hasActiveFilters =
		props.search.length > 0 || props.status !== 'all' || props.sort !== 'recent';
	...
	return (
		...
		>
			...
			<Select
				...
				value={props.status}
				onChange={(event) =>
					props.onStatusChange(event.target.value as TenantStatusFilter)
				}
			...
			<Select
				...
				value={props.sort}
				onChange={(event) =>
					props.onSortChange(event.target.value as TenantSortOption)
				}
				options={sortOptions}
