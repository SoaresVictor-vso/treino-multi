...
export default function UsersFilters(props: UsersFiltersProps) {
	const hasActiveFilters =
		props.search.length > 0 || props.role !== 'all' || props.sort !== 'recent';
	...
	return (
		<FiltersPanel
			...
			summary={
				<>
					{props.visibleCount} de {props.totalCount} usuários exibidos
				</>
			}
		>
			<Input
			...
			<Select
				...
				value={props.role}
				onChange={(event) =>
					props.onRoleChange(event.target.value as UserRoleFilter)
				}
				options={roleOptions}
			...
			<Select
				...
				value={props.sort}
				onChange={(event) =>
					props.onSortChange(event.target.value as UserSortOption)
				}
				options={sortOptions}
