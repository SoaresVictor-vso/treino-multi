...
const mapTenantDetailsToFormValues = (
...
const mapTenantFormToUpdatePayload = (
...
export default function TenantsPage() {
	...
	const [statusFilter, setStatusFilter] =
		React.useState<TenantStatusFilter>('all');
	const [sortBy, setSortBy] = React.useState<TenantSortOption>('recent');
	const [modalState, setModalState] = React.useState<TenantModalState | null>(
		null,
	);
	...
	const requestTenants = async (searchValue: string) => {
		...
		if (!result.success || !result.data) {
			setLoadError(
				result.error || 'Nao foi possivel carregar a listagem de tenants.',
			);
			setIsLoading(false);
			return;
		}
	...
	const filteredTenants = tenants
		...
		.sort((left, right) => {
			if (sortBy === 'name-asc') {
				return (left.tradeName || left.name).localeCompare(
					right.tradeName || right.name,
				);
			}

			if (sortBy === 'name-desc') {
				return (right.tradeName || right.name).localeCompare(
					left.tradeName || left.name,
				);
			}
			...
			setModalState((current) => {
				...
				if (!result.success || !result.data) {
					return {
						...
						detailsError:
							result.error || 'Nao foi possivel carregar os dados do tenant.',
					};
				}
			...
			<div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
					...
					<div className="mt-3 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
						<div className="max-w-2xl space-y-3">
							...
							<p className="max-w-xl text-sm leading-6 text-on-surface-variant md:text-base">
								A tela agora usa o service real de tenants e segue a direcao visual das
								referencias em{' '}
				...
				<section className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
					<MetricCard
						label="Ativos"
						value={activeCount}
						description="Prontos para uso no ecossistema."
					/>
					<MetricCard
						label="Inativos"
						value={inactiveCount}
						description="Sem acesso, mas ainda nao arquivados."
					/>
					<MetricCard
						label="Arquivados"
						value={archivedCount}
						description="Registros mantidos apenas para historico."
					/>
				</section>
			</div>
				...
				<ModalTenant
					key={
						modalState.mode === 'create'
							? 'create-tenant'
							: `${modalState.mode}-${modalState.tenantId}`
					}
					state={modalState}
...
function ModalTenant(props: {
	...
	onCreate: (
		tenant: CreateTenantDto,
		admin: CreateTenantAdminDto,
	) => Promise<ApiResponse<TenantListItemDto>>;
	onUpdate: (
		tenantId: string,
		tenant: UpdateTenantDto,
	) => Promise<ApiResponse<TenantListItemDto>>;
	...
	const [tenantErrors, setTenantErrors] = React.useState<
		Partial<Record<keyof CreateTenantDto, string>>
	>({});
	const [adminErrors, setAdminErrors] = React.useState<
		Partial<Record<keyof CreateTenantAdminDto, string>>
	>({});
	...
	const isLoadingDetails =
		props.state.mode === 'create' ? false : props.state.isLoadingDetails;
	const detailsError =
		props.state.mode === 'create' ? null : props.state.detailsError;
	...
	const validateTenantForm = async () => {
		try {
			...
			const values = isEditMode
				? mapTenantFormToUpdatePayload(effectiveTenant)
				: effectiveTenant;
			...
			setTenantErrors(
				mapYupErrors(error) as Partial<Record<keyof CreateTenantDto, string>>,
			);
			return false;
		}
	};
	...
	const validateAdminForm = async () => {
		try {
			...
			setAdminErrors(
				mapYupErrors(error) as Partial<Record<keyof CreateTenantAdminDto, string>>,
			);
			return false;
		}
	};
			...
			conf: {
				...
				children: (
					<CompanyForm
						...
						setTenant={setTenantDraft}
						errors={tenantErrors}
						disabled={isViewMode}
						isSlugLocked={isEditMode}
			...
			>
				<div className="grid gap-4 md:grid-cols-2">
					...
					<Input
						...
						hint={
							props.isSlugLocked
								? 'Slug bloqueado apos a criacao do tenant.'
								: 'Usado na identificacao tecnica do tenant.'
						}
						value={props.tenant.slug}
						error={props.errors.slug}
						disabled={props.disabled || props.isSlugLocked}
						onChange={(event) =>
							updateField('slug', event.target.value.toLowerCase().trim())
						}
					/>
					<Input
					...
					<Input
						...
						onChange={(event) =>
							updateField('cnpj', event.target.value.replace(/\D/g, '').slice(0, 14))
						}
					/>
			...
			>
				<div className="grid gap-4 md:grid-cols-2">
					...
					<Input
						...
						hint={
							props.disabled
								? 'A senha nao e exibida por seguranca.'
