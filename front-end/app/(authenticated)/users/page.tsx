...
import UsersFilters, {
	type UserRoleFilter,
	type UserSortOption,
} from '@/app/(open)/login/UsersFilters';
...
export default function UsersPage() {
	...
	const [selectedUser, setSelectedUser] = React.useState<UserListItemDto | null>(
		null,
	);
	...
	useEffect(() => {
		...
		void (async () => {
			...
			const resolvedTenantId =
				sessionUser?.tenantId ?? (tenantFilter || undefined);
			...
			if (!result.success || !result.data) {
				setLoadError(
					result.error || 'Nao foi possivel carregar a listagem de usuarios.',
				);
				setIsLoading(false);
				return;
			}
	...
	const handleUserSaved = async () => {
		...
		if (refreshed.success && refreshed.data) {
			...
			setLoadError(
				refreshed.error || 'Nao foi possivel recarregar a listagem de usuarios.',
			);
		}
		setIsLoading(false);
	};
	...
	return (
		<div className="space-y-6 p-4">
			...
			{isLoading && (
				<p className="text-sm text-on-surface-variant">Carregando usuários...</p>
			)}
...
function countUsersWithRole(users: UserListItemDto[], role: Role) {
	return users.filter((user) =>
		user.userRoles.some((userRole) => userRole.role === role),
	).length;
}
...
function UserMetrics(props: { users: UserListItemDto[]; isOrgActor: boolean }) {
	...
	return (
		<section
			className={`grid gap-3 sm:grid-cols-2 ${isOrgActor ? 'xl:grid-cols-4' : 'xl:grid-cols-3'}`}
		>
			{isOrgActor && (
				<MetricCard
					label="Organização"
