...
export class SeedOrgAdmin1777333212536 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		const sysPasswordHash = sysPassword
			? await bcrypt.hash(sysPassword, 12)
			: null;
		...
		if (sysPassword) {
           ...
           VALUES ($1, NULL, 'organization', $2, false) RETURNING id`,
				...
				console.log(
					`[Seed] usuário de sistema criado com email: ${SYSTEM_USER_EMAIL}`,
				);
			} else {
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(`DELETE FROM persons WHERE email = $1`, [
			SYSTEM_USER_EMAIL,
		]);
