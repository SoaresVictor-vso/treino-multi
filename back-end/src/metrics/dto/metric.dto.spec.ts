...
describe('Metric DTOs', () => {
  ...
  it('aceita DTO de criação válido', async () => {
    expect(await validate(Object.assign(new CreateMetricDto(), valid()))).toHaveLength(0);
  });
  ...
  it('aceita atualização parcial', async () => {
    expect(await validate(Object.assign(new UpdateMetricDto(), { symbol: 'kg' }))).toHaveLength(0);
  });
});