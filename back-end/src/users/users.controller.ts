...
export class UsersController {
	...
	@ApiOperation({
		summary: 'Lista usuários com paginação e filtros por tenant, nome e role',
	})
	@ApiQuery({
		name: 'tenantId',
		required: false,
		description: "Use 'null' para usuários sem tenant",
	})
	...
	@ApiQuery({
		...
		required: false,
		description:
			'Valor do campo de ordenação do último elemento da página anterior',
	})
	 ...
	 */
	@ApiOperation({
		summary: 'Gera link/token de redefinição de senha (TTL 30 min)',
	})
	@ApiResponse({ status: 201, description: 'Token gerado com sucesso' })
	@ApiResponse({
		status: 403,
		description: 'Acesso negado (requer actor sem tenantId)',
	})
	...
	private ensureActorWithoutTenant(actor: jwtPayloadInterface.JwtPayload): void {
		if (actor.tenantId) {
