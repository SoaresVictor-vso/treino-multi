...
export class UsersService {
	...
	async createManagedUser(
		...
		return this.dataSource.transaction(async (em) => {
			await this.ensurePersonUniqueness(em, dto.email, dto.document ?? null);
	...
	async findManagedUsers(dto: FindUsersQueryDto): Promise<{
		...
		const limit = Math.min(
			100,
			Math.max(1, this.parsePositiveInt(dto.limit, 20)),
		);
	...
	async findManagedUser(id: string, tenantId?: string | null): Promise<User> {
		...
		if (tenantId && user.tenantId !== tenantId)
			throw new ForbiddenException(
				'Acesso negado: usuário não pertence à empresa.',
			);
	...
	async updateManagedUser(
		...
		if (dto.email !== undefined || dto.document !== undefined) {
			await this.ensurePersonUniqueness(
				...
				dto.document !== undefined ? dto.document : (user.person.document ?? null),
				user.personId,
			);
		}
	...
	async generatePasswordResetToken(
		...
		const token = jwt.sign({ sub: userId, purpose: 'password-reset' }, secret, {
			expiresIn: '30m',
		});
		await this.auditLogService.logCriticalOperation({
			tenantId: user.tenantId,
	...
	async resetPassword(
		...
		const usedTokenHash = crypto.createHash('sha256').update(token).digest('hex');
		...
		const alreadyUsed =
			await this.auditLogService.isPasswordResetTokenAlreadyUsed(usedTokenHash);
		if (alreadyUsed) {
