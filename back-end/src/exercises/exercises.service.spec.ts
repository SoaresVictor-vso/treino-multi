...
describe('ExercisesService', () => {
  ...
  beforeEach(async () => {
    const module = await Test.createTestingModule({ providers: [
      ExercisesService,
      { provide: getRepositoryToken(Exercise), useValue: { find: jest.fn(), findOne: jest.fn(), create: jest.fn((dto) => dto), save: jest.fn(), remove: jest.fn() } },
  ...
  it('lista exercício com métricas e remove existente', async () => {
    const exercise = makeExercise();
    ...
    expect(exerciseRepo.remove).toHaveBeenCalledWith(exercise);
  });
