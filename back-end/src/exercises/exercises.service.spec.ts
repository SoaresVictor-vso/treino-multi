...
describe('ExercisesService', () => {
	...
	it('cria exercício após validar suas métricas', async () => {
		...
		await expect(
			service.create({ name: exercise.name, metric1Id: 1 }),
	...
	it('salva o nome atualizado em uppercase', async () => {
		const exercise = makeExercise();
		metricRepo.existsBy.mockResolvedValue(true);
		exerciseRepo.findOne.mockResolvedValue(exercise);
		exerciseRepo.save.mockResolvedValue({ ...exercise, name: 'SUPINO RETO' });

		await service.update(exercise.id, { name: 'Supino reto' });
	...
	it('rejeita exercício quando uma métrica não existe', async () => {
		...
		await expect(
			service.create({ name: 'Agachamento', metric1Id: 99 }),
	...
	it('retorna alterações ativas e IDs removidos desde uma data', async () => {
		...
		expect(exerciseRepo.find).toHaveBeenNthCalledWith(
			1,
