...
describe('ExercisesController', () => {
  ...
  it('busca alterações desde a data informada', async () => {
    ...
    const changes = { exercises: [], deletedIds: [2], syncedAt: '2026-07-31T10:05:00.000Z' };
  ...
  it('busca todos os exercícios quando since é nulo', async () => {
    const changes = { exercises: [{ id: 1, name: 'Agachamento' }], deletedIds: [], syncedAt: '2026-07-31T10:05:00.000Z' };
    service.findChangesSince.mockResolvedValue(changes);

    await expect(controller.findChanges({ since: null })).resolves.toEqual(changes);
