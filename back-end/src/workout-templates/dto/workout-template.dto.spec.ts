...
describe('Workout template DTOs', () => {
	it('aceita template e atividade válidos', async () => {
		const dto = Object.assign(new CreateWorkoutTemplateDto(), {
			...
			description: 'Base',
			activities: [
				Object.assign(new ActivityDto(), { exerciseId: 1, type1: 'v', pse: 8 }),
			],
		});
	...
	it('rejeita UUID e tipos de registro inválidos', async () => {
		const activity = Object.assign(new ActivityDto(), {
			exerciseId: 0,
			type1: 'p',
			type2: 'x',
			pse: 8,
		...
		const template = Object.assign(new CreateWorkoutTemplateDto(), {
			tenantId: 'inválido',
		});
	...
	it('aceita atualização parcial', async () => {
		expect(
			await validate(
				Object.assign(new UpdateWorkoutTemplateDto(), { name: 'Treino B' }),
			),
		).toHaveLength(0);
	});
});
