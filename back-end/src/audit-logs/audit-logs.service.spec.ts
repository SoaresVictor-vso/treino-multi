...
describe('AuditLogService', () => {
		...
		const module: TestingModule = await Test.createTestingModule({
			providers: [
				AuditLogService,
				{
					...
					useFactory: makeRepoMock,
				},
				{
					provide: getRepositoryToken(CriticalOperationLog),
					useFactory: makeRepoMock,
				},
				{
					provide: getRepositoryToken(PasswordChangeLog),
					useFactory: makeRepoMock,
				},
			],
		}).compile();
	...
	describe('findAuthenticationLogs()', () => {
		...
		it('deve aplicar filtro de tenantId quando fornecido', async () => {
			...
			expect(qb.andWhere).toHaveBeenCalledWith('al.tenant_id = :tenantId', {
				tenantId: 'tenant-uuid-1',
			});
		});
	...
	describe('findCriticalOperationLogs()', () => {
		...
		it('deve aplicar filtro de userId quando fornecido', async () => {
			...
			expect(qb.andWhere).toHaveBeenCalledWith('col.user_id = :userId', {
				userId: 'actor-uuid-1',
			});
		});
	});
	...
	describe('findPasswordChangeLogs()', () => {
		...
		it('deve aplicar filtros de userId e tenantId simultaneamente', async () => {
			...
			expect(qb.andWhere).toHaveBeenCalledWith('pcl.user_id = :userId', {
				userId: 'user-uuid-1',
			});
			expect(qb.andWhere).toHaveBeenCalledWith('pcl.tenant_id = :tenantId', {
				tenantId: 'tenant-uuid-1',
			});
		});
	});
