...
describe('AuthService password login and sessions', () => {
	...
	beforeEach(async () => {
		...
		tenantUser = {
			...
			tenantId: 'tenant-id',
			tenant: { slug: 'studio' },
			userRoles: [{ role: Role.TENANT_TRAINER, deletedAt: null }],
		};
		personRepo = { findOne: jest.fn().mockResolvedValue(person) };
		userRepo = {
			find: jest.fn().mockResolvedValue([athlete, tenantUser]),
			findOne: jest.fn(),
		};
		refreshRepo = {
			findOne: jest.fn(),
		...
		service = new AuthService(
			userRepo as any,
			{} as any,
			{ transaction: (action: any) => action(manager) } as any,
			jwt as any,
			{} as any,
			audit as any,
			{} as any,
			{
				execute: async (query: any) => {
					if (query.script === 'loginUsers') {
						const matching = await personRepo.findOne({
							where: { email: query.email },
						});
						return matching
							? userRepo.find({
									where: { personId: matching.id, isActive: true },
									relations: ['userRoles', 'person'],
								})
							: [];
					}
					if (query.script === 'logout') {
						const stored = await refreshRepo.findOne({
							where: { tokenHash: query.tokenHash },
						});
	...
	it('normalizes email and selects the standalone athlete by default', async () => {
		expect(
			await service.validateUser(' ATHLETE@EXAMPLE.TEST ', 'correct-password'),
		).toBe(athlete);
	...
	it('uses the standalone account when a person has multiple accounts, or the sole account', async () => {
		expect(await service.validateUser(person.email, 'correct-password')).toBe(
			athlete,
		);
	...
	it('does not allow password login for an OAuth-only account', async () => {
		...
		expect(
			await service.validateUser(person.email, 'correct-password'),
		).toBeNull();
	});
	...
	it('stores only refresh and family hashes and limits an unremembered login to eight hours', async () => {
		...
		const family = manager.save.mock.calls.find(
			([entity]) => entity === SessionFamily,
		)![1];
		const refresh = manager.save.mock.calls.find(
		...
		expect(family.absoluteExpiresAt.getTime() - Date.now()).toBeLessThanOrEqual(
			8 * 60 * 60_000,
		);
	...
	it('gives remembered refreshes a rolling 30 day deadline without a family deadline', async () => {
		...
		expect(refresh.expiresAt.getTime() - Date.now()).toBeLessThanOrEqual(
			30 * 86_400_000,
		);
		expect(refresh.expiresAt.getTime() - Date.now()).toBeGreaterThan(
			29 * 86_400_000,
		);
	});
	...
	it('rejects unknown logins and revokes the entire family on logout', async () => {
		...
		await expect(
			service.login({ login: 'missing@test.local', password: 'correct-password' }),
		...
		expect(familyRepo.update).toHaveBeenCalledWith(
			{ familyHash: 'stored-family-hash' },
			{ revokedAt: expect.any(Date) },
		);
		refreshRepo.findOne.mockResolvedValue(null);
		await expect(service.logout('missing')).rejects.toBeInstanceOf(
			NotFoundException,
		);
	});
});
...
describe('AuthService OAuth login audit and lookup', () => {
	...
	const db = {
		query: jest.fn(),
		transaction: async (action: any) => action(manager),
	};
	...
	it('loads the linked account and its roles in one query, then logs success', async () => {
		...
		expect(audit.logAuthentication).toHaveBeenCalledWith({
			tenantId: null,
			context: 'standalone',
			success: true,
			loginUsed: hash(person.email),
			ipAddress: '127.0.0.1',
		});
	});
	...
	it('logs a verified identity that cannot be linked as a failed login', async () => {
		...
		await expect(
			service.loginOAuth('google' as any, 'credential', false, '127.0.0.1'),
